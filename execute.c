...
/*ERCH*/
/*printf ("Recursive_Capture x=%d, y=%d\n", x, y);
	return(TRUE); */
}
...
/*ERCH*/
	printf ("\n\nDo_Move x=%d, y=%d, color=", x+1, y+1);
	if (color && (int)color==-16)/*BLACK*/
	{
		GLOBAL_BOARD[x][y]=1;
		printf ("BLACK\n");
	}
	if (color && (int)color==15)/*WHITE*/
	{
		GLOBAL_BOARD[x][y]=-1;
		printf ("WHITE\n");
	}
...
/*ERCH  -- logic for zone*/
	printf ("New stone placed in zone: ");
...
/*ERCH  This is how I want to format for use by Tensor Flow*/
	int k=0;
        printf("{");
        for (int i=0; i<19;i++) {
                for (int j=0;j<19;j++) {
			if (GLOBAL_BOARD[j][i]) {
					...
					printf(", "); 
                                ...
                                printf ("{%d,%d,%d}", j+1, i+1, GLOBAL_BOARD[j][i]);
				k++;
			}
                }
        }
...
/*ERCH  Draw a goban with top and side markers.*/
        printf("  ");
        for (int i=1;i<20;i++) {
                printf("%2d ", i);
        }
        ...
        for (int i=0; i<19;i++) {
                ...
                for (int j=0;j<19;j++) {
                        ...
                        if (GLOBAL_BOARD[j][i] == 0)
                                printf (".  ");
                        else if (GLOBAL_BOARD[j][i] == 1)
                                printf ("@  ");
                        else if (GLOBAL_BOARD[j][i] == -1)
                                printf ("O  ");
                        else
                                printf ("%d  ", GLOBAL_BOARD[j][i]);
